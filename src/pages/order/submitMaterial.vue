<template>
  <div class="submitOrderWrap materialWrap" :class="[isIphoneX ? 'phoneXWrap' : '']">
    ...
    <!-- 转让商标表单 -->
    <div class="submitMain" v-if="tabIndex === 0">
      <div class="orderForm queryCls">
        <h3>请填写商标信息</h3>
        <div class="inputItem">
          <div class="des"><span>*</span>注册号</div>
          <input type="text" v-model="form.regNo" @blur="searchTrademark" :maxlength="12" placeholder-class="placeholder" placeholder="请输入商标注册号" />
        </div>
        <div class="inputItem">
        ...
        <div class="inputItem" style="border: none;margin-bottom: 0;">
          <div class="des"><span>*</span>商标证书</div>
          <p>请上传以下材料的扫描件或清晰照片（至少一种）</p>
          <h3>商标注册证：</h3>
          <div class="photoWrap">
            <div class="photo" v-if="imgArr.length < 10" @click="chooseImage(1)">
            ...
            <div class="pic" v-for="(item, index) in imgArr" :key="index" @click="preview">
              <img class="img" :src="item.fileUrl" :data-index="index+1" data-imgtype="imgArr" mode="aspectFill" alt="">
              <p @click.stop="chooseImage(1, index+1)">重新上传</p>
              <img src="~assets/img/icon/close.png" @click.stop="delImg(index, 1)" class="close" alt="">
            </div>
          </div>
          <h3>商标注册证变更证明：</h3>
          <div class="photoWrap">
            <div class="photo" v-if="trademarkChangeImg.length < 10" @click="chooseImage(2)">
              <img mode="aspectFill" v-if="false" src="~assets/img/order/timg1.jpg" alt="">
            </div>
            <div class="pic" v-for="(item, index) in trademarkChangeImg" :key="index" @click="preview">
              <img class="img" :src="item.fileUrl" :data-index="index+1" data-imgtype="trademarkChangeImg" mode="aspectFill" alt="">
              <p @click.stop="chooseImage(2, index+1)">重新上传</p>
              <img src="~assets/img/icon/close.png" @click.stop="delImg(index, 2)" class="close" alt="">
            </div>
          </div>
          <h3>商标受理通知书：</h3>
          <div class="photoWrap">
            <div class="photo" v-if="transationImg.length < 10" style="margin-bottom: 0;"  @click="chooseImage(3)">
              <img mode="aspectFill" v-if="false" src="~assets/img/order/timg1.jpg" alt="">
            </div>
            <div class="pic" style="margin-bottom: 0;" v-for="(item, index) in transationImg" :key="index" @click="preview">
              <img class="img" :src="item.fileUrl" :data-index="index+1" data-imgtype="transationImg" mode="aspectFill" alt="">
              <p @click.stop="chooseImage(3, index+1)">重新上传</p>
              <img src="~assets/img/icon/close.png" @click.stop="delImg(index, 3)" class="close" alt="">
            </div>
          </div>
    ...
    <div class="selfInfoWrap" v-if="tabIndex === 1" >
      ...
      <div class="orderForm" :style="{minHeight: selfInfoWrapHeight + 'px'}">
        <h3>请填写转让主体信息</h3>
        <!-- <div class="inputItem" style="overflow: hidden;">
          <div class="des">主体类型</div>
          <div class="subjectWrap">
            <span class="radioWrap">
              <span v-if="form.subjectType == 1" class="radio"></span>
            </span>
            <span class="typeDes">个人</span>
          </div>
          <div class="subjectWrap">
            <span class="radioWrap">
              <span v-if="form.subjectType == 2" class="radio"></span>
            </span>
        ...
        <template v-if="form.subjectType == 1">
          ...
          <!-- <div class="inputItem idCardWrap" style="border: none;margin-bottom:0;">
            ...
            <div class="idCard">
              <div class="frontWrap">
                <div class="front">
                  <img mode="aspectFill" v-if="form['1133']" :src="form['1133'].fileUrl" alt="">
                  <img mode="aspectFill" v-if="!form['1133']" src="~assets/img/order/timg.jpg" alt="">
                  <div class="des">上传身份证人像面</div>
                </div>
              </div>
              <div class="reverseWrap">
                <div class="reverse">
                  <img mode="aspectFill"  v-if="form['1132']" :src="form['1132'].fileUrl" alt="">
                  <img mode="aspectFill"  v-if="!form['1132']" src="~assets/img/order/timg.jpg" alt="">
    ...
    <div class="selfInfoWrap" id="selfInfoWrap"  v-if="tabIndex === 2">
      <div class="orderForm" :style="{minHeight: selfInfoWrapHeight + 'px'}">
        ...
        <template v-if="form.assigneeType == 1">
          ...
          <div class="inputItem">
            <div class="des"><span>*</span>身份证号</div>
            <input type="text" v-model="form.assigneeIdno" placeholder-class="placeholder" placeholder="请输入身份证号" />
          </div>
          <div class="inputItem idCardWrap" style="border: none;margin-bottom:0;">
            <div class="des"><span>*</span>身份证扫描件</div>
            <div class="idCard">
    ...
    <div class="nextStepWrap queryCls">
      ...
      <button type="button" v-if="!backIndexBtn" :class="['nextStepBtn', btnState ? '' : 'btnDisabled']" @click="nextStep">下一步</button>
      <div v-if="backIndexBtn" :class="['nextStepBtn', 'indexBtnWrap', btnState ? '' : 'btnDisabled']">
        <div class="backIndex" @click="backIndex">返回首页</div>
        <div class="nextBtn" @click="nextStep">下一步</div>
      </div>
...
export default {
  ...
  data () {
    return {
      tabIndex: 0,
      cateIdData: cateIdData.cateIdData,
      checkedTrademark: [],
  ...
  onShow () {
    ...
    if (pages.length <= 1) {
      this.backIndexBtn = true;
    } else {
      this.backIndexBtn = false;
    }
  ...
  methods: {
    ...
    async init () {
      ...
      let originalData = await this.API.orderBs.selectOrderBs({orderSn: this.orderSn});
      Object.assign(this.form, originalData);
      if (this.form.orderBsFiles && this.form.orderBsFiles.length) {
        this.form.orderBsFiles.forEach(item => {
          item.orderBsFiles.forEach(it => {
            it.fileType = item.fileType;
          })
          switch (item.fileType+'') {
            case '1': // 商标证
              Array.prototype.push.apply(this.imgArr, item.orderBsFiles);
              break;
            case '2': // 商标证
              Array.prototype.push.apply(this.trademarkChangeImg, item.orderBsFiles);
              break;
            case '3': // 商标证
              Array.prototype.push.apply(this.transationImg, item.orderBsFiles);
              break;
            case '1031': // 办证补充材料
              this.form['1031'] = item;
              break;
            case '1099': // 其他材料
              ...
              break;
            default:
              this.$set(this.form, item.fileType+'', item.orderBsFiles[0]);
              break;
          }
        })
      }
    ...
    preview (e) {
      if (e.target) {
        console.log('e.target', e.target, e.target.dataset.imgtype)
        let index = e.target.dataset.index;
        let urls = this[e.target.dataset.imgtype].map(item => item.fileUrl);
        if (index) {
          wx.previewImage({
    ...
    validaFunc () {
      ...
      // validator.add(this.imgArr.join(','), 'isNotEmpty', '请上传商标证书')
    ...
    firstStep () {
      ...
      if (!this.imgArr.length&&!this.trademarkChangeImg.length&&!this.transationImg.length) {
        this.tips('请上传商标证书');
        wx.pageScrollTo({scrollTop: 400, duration: 10});
        return;
      }
      this.form.cls = this.checkedTrademark.join(',');
      this.form.orderBsFileVos = [...this.imgArr, ...this.transationImg, ...this.trademarkChangeImg];
      this.form.imgArr = this.imgArr;
      this.form.trademarkChangeImg = this.trademarkChangeImg;
      this.form.transationImg = this.transationImg;
      this.materialArr[this.orderSn] = this.form;
      this.$storage.set('materialArr', this.materialArr);
      this.$router.push({path: '/pages/order/submitMaterial2', query: {orderSn: this.orderSn}});
      // this.tabIndex = 1;
      // this.tabIndex = 1;
    ...
    thirdStep () {
      ...
      this.materialArr[this.orderSn] = this.form;
      this.$storage.set('materialArr', this.materialArr);
      this.$router.replace(`/pages/order/confirmMaterial?orderSn=${this.orderSn}`);
    },
    nextStep () {
      if (!this.tabIndex) {
        this.firstStep()
   ...
   // fileType文件类型， index是图片索引+1
    ...
    async searchTrademark () {
      if (this.form.regNo.length > 3) {
        ...
        console.log('fiofiofio', this.cateIdData, this.checkedTrademark)
      }
    },
  },
  watch: {
...
.materialWrap{padding: 0 0 208px;position: relative; overflow-x: hidden;
  ...
  position: fixed;bottom: 0;left: 0;width: 100%;z-index: 10;
    ...
    .indexBtnWrap{display: flex;
      >div{flex: 1;color: #fff;box-sizing: content-box;}
      .backIndex{background: #fff;border-radius: 45px 0 0 45px;color: #F64744;border: 1px solid #F64744;}
